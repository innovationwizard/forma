  ...
  if (!canEdit) {
    return (
      <p className="text-foreground/60 text-xs">
        Tu rol es solo lectura. El servidor lo aplica; las solicitudes de mutación
        responderían <code>403 prohibido</code>.
      </p>
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-3">
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        <Field label="Tipo de cambio anclado (GTQ por USD)">
          <input
            type="text"
            inputMode="decimal"
            value={tc}
        ...
        <Field label="Tasa de IVA (fracción, ej. 0.12 = 12%)">
          <input
            type="text"
            inputMode="decimal"
            value={iva}
      ...
      {dirty ? (
        <Field label="Motivo (obligatorio — estas tasas son fundamentales)">
          <input
            type="text"
            required
            value={reason}
            ...
            disabled={pending}
            placeholder="ej. publicación BANGUAT + visto bueno de Federico"
            className={inputClass}
          />
        </Field>
      ) : null}
