...
export function HouseCard({ row }: HouseCardProps) {
  ...
  return (
    <Link
      ...
      aria-label={`Abrir detalle de ventas de ${row.name}`}
    >
      ...
      <div className="text-foreground/70 text-xs">
        {row.buyer != null ? (
          ...
          <span className="text-foreground/40 italic">
            {row.status === "SOLD" ? "(comprador no vinculado — faltan datos)" : "sin comprador aún"}
          </span>
        )}
      </div>

      <dl className="text-foreground/80 grid grid-cols-2 gap-x-3 gap-y-1 text-xs">
        <dt className="text-foreground/50">Precio</dt>
        <dd className="text-foreground text-right tabular-nums">
          {row.salePriceSinIvaUsd != null ? formatUsd(row.salePriceSinIvaUsd) : "—"}
        </dd>
        <dt className="text-foreground/50">Pagado a la fecha</dt>
        <dd className="text-foreground text-right tabular-nums">{formatUsd(row.totalPaidUsd)}</dd>
        <dt className="text-foreground/50">Mes de venta</dt>
        <dd className="text-foreground text-right tabular-nums">
          {row.saleMonth != null ? `M${row.saleMonth}` : "—"}
        </dd>
        <dt className="text-foreground/50">Mes de entrega</dt>
        <dd className="text-foreground text-right tabular-nums">
      ...
      {planned > 0 ? (
        <div className="text-foreground/60 flex items-center justify-between text-[10px] tabular-nums">
          <span>{formatPct(completion)} pagado</span>
          {row.dataIncomplete ? (
            <span className="text-amber-700">▲ datos incompletos</span>
          ) : null}
        </div>
      ) : null}
    </Link>
