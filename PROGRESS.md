...
- Batch 5 (XLSX parser) is unblocked — workbook is already in `docs/CONCILIACIÓN/` and the schema has every entity needed for seeding in Batch 6.
...
### Batch 13a — CONCILIACIÓN bronze + silver + G&T adapter (2026-05-27)
...
All 4 design decisions signed off in `CONCILIACIÓN_DESIGN.md` §8.
...
- `CONCILIACIÓN_DESIGN.md` (repo root, sign-off artifact + progress tracker §9)
- `prisma/migrations/20260527193908_batch_13a_conciliación_bronze_silver/migration.sql` (158 lines: 4 tables + 6 enums + indexes + FKs)
- `prisma/migrations/20260527194107_batch_13a_conciliación_rls/migration.sql` (931 lines: regenerated full RLS for all matrix resources)
...
**Tech debt surfaced (logged in CONCILIACIÓN_DESIGN.md §9; deferred to 13b/13c when other banks land)**
...
- PROMERICA / BAC / INDUSTRIAL parsing — by design (stubs ship disabled until real samples arrive in `docs/CONCILIACIÓN/`).
...
### Batch 13b — CONCILIACIÓN gold additions + classification queue (2026-05-27)
...
### Batch 13c — CONCILIACIÓN per-house reconciliation UI (2026-05-28)
...
- `src/lib/queries/casa-conciliación.ts` — composite query
...
- `src/app/(app)/casa/[id]/conciliación/page.tsx` — page (server component); nested payment sub-rows under each month; cumulative balance column highlighted red when behind
...
- `src/components/dashboard/RevenueBlock.tsx` — RvUnit names now `<Link>` to `/casa/[id]/conciliación`
...
- `pnpm build` — `/casa/[id]/conciliación` registered as `ƒ` (dynamic)
...
- The flow where an Analyst classifies an inflow → RvPayment from `/inbox` → it appears on `/casa/[id]/conciliación` was exercised in Batch 13b's smoke (the FK was verified) but not in 13c (today's smoke is read-only). The data flow is correct by construction (the `loadCasaConciliación` query reads `RvPayment` rows by `rvUnitId`); confirming visually waits for the same Batch 19 gate.
...
### Batch 13d — CONCILIACIÓN check-register adapter (2026-05-28)
...
- Manual walkthrough for Ronny (Analyst) — separate session focused on `/inbox` + `/casa/[id]/conciliación` + `/entry/new` workflows. Per `feedback_intent_vs_implementation` the daily-driver experience needs his sign-off.
...
- **Conciliación (`src/components/casa/reconciliation-style.ts`, `src/app/(app)/casa/[id]/conciliación/page.tsx`)**: status labels (Coincide/Sobrepago/Subpago/Omitido/Por venir/Inesperado), CONCILIACIÓN MENSUAL heading.
...
| `docs/CONCILIACIÓN/<filename> — MANIFEST.md` × 13                                                                                                        | Per-file structural manifests (sheets, dimensions, headers, totals, formula density). Each sorts directly before its source file in `ls`. Re-generate via the inspector script `/tmp/forma_inspect.py` if any source file changes. | pre-Batch-1                             |
